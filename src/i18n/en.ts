export const en = {
  common: {
    add: 'Add',
    edit: 'Edit',
    delete: 'Delete',
    cancel: 'Cancel',
    save: 'Save',
    search: 'Search...',
    done: 'Done',
    close: 'Close',
    confirm: 'Confirm',
    loading: 'Loading...',
    error: 'Error',
    success: 'Success',
    yes: 'Yes',
    no: 'No',
    or: 'or',
    items: 'items',
    item: 'item',
    restore: 'Restore',
    copy: 'Copy',
    selected: 'selected',
    undo: 'Undo',
    redo: 'Redo',
  },
  home: {
    title: 'My Lists',
    newList: 'New List',
    emptyState: 'No lists yet. Create your first one!',
    deleteList: 'Delete List',
    deleteListConfirm: 'Are you sure you want to delete this list?',
    listItems: '{{count}} items',
    lastUpdated: 'Updated {{time}}',
    listOptions: 'List options',
    listDeleted: '"{{name}}" deleted',
    swipeToDeleteHint: 'Swipe a list left to delete it',
  },
  list: {
    addItem: 'Add item...',
    addMultiple: 'Add multiple items',
    pasteListHint: 'Paste your list here...\nOne item per line, or separated by commas',
    emptyState: 'Your list is empty',
    checkedItems: 'Checked Items',
    outOfStock: 'Out of Stock',
    pendingNextTrip: 'Pending for Next Trip',
    markOutOfStock: 'Out of Stock',
    markChecked: 'Checked',
    deleteItem: 'Delete',
    editItem: 'Edit Item',
    itemName: 'Item name',
    quantity: 'Quantity',
    price: 'Price (optional)',
    category: 'Category',
    uncategorized: 'Uncategorized',
    startShopping: 'Start Shopping',
    shareList: 'Share List',
    clearChecked: 'Clear Checked',
    uncheckAll: 'Uncheck All',
    resetShopping: 'Reset Shopping',
    resetShoppingConfirm: 'This will uncheck all items and reset out-of-stock items. Ready to start fresh?',
    deleteList: 'Delete List',
    deleteListConfirm: 'Are you sure you want to delete this list? This action cannot be undone.',
    itemAlreadyExists: 'Item already exists in the list',
    duplicatesSkipped: 'Some items were skipped (already in list)',
    checkItem: 'Mark as picked',
    uncheckItem: 'Mark as not picked',
    addToExisting: 'Add to existing item',
    quantityIncreased: 'Quantity updated',
    aiCategorizing: 'Improving categories with AI...',
    aiFailed: 'AI categorization did not run - items were categorized by keyword',
    favoritesAdded: '{{count}} favorites added',
    itemDeleted: '"{{name}}" deleted',
  },
  shopping: {
    title: 'Shopping Mode',
    complete: 'Complete Shopping',
    itemsLeft: '{{count}} items left',
    allDone: 'All done!',
    tapToCheck: 'Swipe an item right when you have it',
    swipeForActions: 'Swipe left if it is out of stock - or tap ⋯ for more',
    gotIt: 'Got it',
    addItem: 'Add an item',
    itemActions: 'More for {{name}}',
    backToList: 'Back on the list',
    remembered: 'Forgot something? Add it here without leaving the store.',
    exitShopping: 'Exit Shopping Mode',
    outOfStockItems: '{{count}} items out of stock',
  },
  categories: {
    title: 'Categories',
    addNew: 'Add Category',
    editCategory: 'Edit Category',
    deleteCategory: 'Delete Category',
    deleteCategoryConfirm: 'Items in this category will be moved to "Other"',
    categoryName: 'Category Name',
    categoryNameEn: 'Name (English)',
    categoryNameHe: 'Name (Hebrew)',
    categoryColor: 'Color',
    categoryIcon: 'Icon',
    keywords: 'Keywords',
    keywordsHint: 'Comma-separated keywords for auto-categorization',
    defaultUnit: 'Default Unit',
    defaultQuantity: 'Default Quantity',
    fruits: 'Fruits',
    vegetables: 'Vegetables',
    dairy: 'Dairy',
    meat: 'Meat',
    bakery: 'Bakery',
    frozen: 'Frozen',
    beverages: 'Beverages',
    snacks: 'Snacks',
    household: 'Household',
    personalCare: 'Personal Care',
    baking: 'Baking',
    canned: 'Canned',
    other: 'Other',
    defaultSection: 'Default',
    customSection: 'Custom',
    shoppingOrder: 'Shopping order',
    shoppingOrderHint: 'Drag a category by its handle - or focus the handle and use the arrow keys - to match the way you walk the store. Shopping mode follows this order.',
    reorder: 'Reorder',
    moveUp: 'Move up',
    moveDown: 'Move down',
    resetOrder: 'Reset order',
  },
  favorites: {
    title: 'Favorites',
    addToFavorites: 'Add to favorites',
    removeFromFavorites: 'Remove from favorites',
    noFavorites: 'No favorites yet. Tap the heart icon on items to add them.',
    selectFavorites: 'Select Favorites',
    selectAll: 'Select All',
    deselectAll: 'Deselect All',
    selectedCount: '{{count}} selected',
    addToList: 'Add to List',
    startFromFavorites: 'From favorites',
    favoritesCount: '{{count}} items',
    addAllToList: 'Add All to List',
    manageFavorites: 'Manage Favorites',
  },
  newList: {
    title: 'Create New List',
    listName: 'List Name',
    listNamePlaceholder: 'e.g., Weekly Groceries',
    startWith: 'Start with',
    emptyList: 'Empty list',
    fromFavorites: 'From favorites',
    copyFromList: 'Copy from existing list',
    selectList: 'Select a list',
    create: 'Create List',
    defaultName: 'Shopping {{date}}',
  },
  settings: {
    title: 'Settings',
    language: 'Language',
    english: 'English',
    hebrew: 'עברית',
    theme: 'Theme',
    themeDark: 'Dark',
    themeLight: 'Light',
    themeSystem: 'System',
    aiCategorization: 'AI Categorization',
    aiDescription: 'Use Google Gemini for smarter item categorization',
    aiDisabled: 'Using keyword matching only',
    apiKey: 'API Key',
    enterApiKey: 'Enter your Gemini API key',
    apiKeySet: 'API key is set',
    testConnection: 'Test Connection',
    testSuccess: 'Connection successful!',
    testFailed: 'Connection failed. Please check your API key.',
    removeKey: 'Remove API Key',
    removeKeyConfirm: 'Are you sure you want to remove your API key?',
    getApiKey: 'Get API Key from Google AI Studio',
    about: 'About',
    version: 'Version',
    clearData: 'Clear All Data',
    clearDataConfirm: 'This will delete all your lists, favorites, and settings. This cannot be undone.',
    installApp: 'Install App',
    installDescription: 'Install this app on your device for a better experience',
    dangerZone: 'Danger Zone',
    model: 'Model',
    modelHint: 'Used for AI categorization',
    changeModel: 'Change',
    chooseModel: 'Choose a model',
    loadingModels: 'Asking your key which models it can use...',
    modelsFailed: 'Could not load the model list',
    noModels: 'This key has no usable models',
    recommended: 'Recommended',
    retry: 'Try again',
    testAI: 'Test',
    testing: 'Testing...',
    testPassed: 'Working - answered in {{ms}} ms',
    aiStatus: 'Connection',
    aiStatusHint: 'Checks the key and the model with a real request',
  },
  units: {
    unit: 'Units',
    unitShort: 'x',
    kg: 'Kilograms',
    kgShort: 'kg',
    g: 'Grams',
    gShort: 'g',
    l: 'Liters',
    lShort: 'L',
    ml: 'Milliliters',
    mlShort: 'ml',
    package: 'Package',
    packageShort: 'pkg',
    dozen: 'Dozen',
    dozenShort: 'dz',
    bunch: 'Bunch',
    bunchShort: 'bunch',
  },
  budget: {
    title: 'Budget',
    total: 'Total',
    estimated: 'Estimated',
    remaining: 'Remaining',
    setBudget: 'Set Budget',
    noBudget: 'No budget set',
    overBudget: 'Over budget by',
    underBudget: 'Under budget by',
  },
  share: {
    title: 'Share List',
    description: 'Share this list with family or friends',
    copyLink: 'Copy Link',
    linkCopied: 'Copied!',
    shareCode: 'Share Code',
    shareCodeHint: 'Share this code with others to let them join your list',
    enterCode: 'Enter share code',
    joinList: 'Join List',
    invalidCode: 'Invalid share code',
    shareVia: 'Share via...',
    importing: 'Importing list...',
    pleaseWait: 'Please wait',
    importSuccess: 'List imported!',
    importFailed: 'Failed to import list',
    importError: 'Import Failed',
    invalidLink: 'Invalid or expired share link',
    openList: 'Open List',
    goHome: 'Go to Home',
    listPreview: 'Items',
    moreItems: 'more items',
  },
  frequent: {
    title: 'Frequently Bought',
    addFromFrequent: 'Quick add from history',
    noFrequent: 'Your frequently bought items will appear here',
  },
  errors: {
    generic: 'Something went wrong',
    networkError: 'Network error. Please check your connection.',
    listNotFound: 'List not found',
    itemNotFound: 'Item not found',
    saveFailed: 'Failed to save',
    loadFailed: 'Failed to load data',
  },
  time: {
    justNow: 'Just now',
    minutesAgo: '{{count}} min ago',
    hoursAgo: '{{count}} hours ago',
    daysAgo: '{{count}} days ago',
    today: 'Today',
    yesterday: 'Yesterday',
  },
};

export type TranslationKeys = typeof en;

